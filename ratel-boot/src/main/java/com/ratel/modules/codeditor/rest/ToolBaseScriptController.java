...
import com.ratel.modules.codeditor.service.ToolBaseScriptService;
...
@Api(tags = "系统：脚本")
@RestController
@RequestMapping("/api/tool/baseScript")
public class ToolBaseScriptController {

    @Autowired
    private ToolBaseScriptService toolBaseScriptService;
    @Autowired
    ...
    public void download(HttpServletResponse response, ToolScriptQueryCriteria criteria) throws IOException {
        toolBaseScriptService.download(toolBaseScriptService.queryAll(criteria), response);
    }
    ...
    public ResponseEntity<Object> getDeptsAndUsers(ToolScriptQueryCriteria criteria, Pageable pageable) {
        List<ToolBaseScript> userDtos = toolBaseScriptService.queryAll(criteria);
        ...
        for (ToolBaseScript userDto : userDtos) {
            Map map = new HashMap();
...
//            } else {
        ...
        return FormsHttpEntity.ok(toolBaseScriptService.queryAll(criteria, pageable));
//            }
//            // 否则取并集
//        } else {
//            result.addAll(deptSet);
...
//        }
    ...
    public ResponseEntity<Object> create(@Validated @RequestBody ToolBaseScript resources) {
        // checkLevel(resources);
        // 默认密码 123456
//        resources.setPassword(passwordEncoder.encode("123456"));
        return FormsHttpEntity.ok(toolBaseScriptService.create(resources));
    }
    ...
    public ResponseEntity<Object> update(@Validated(ToolBaseScript.Update.class) @RequestBody ToolBaseScript resources) {
        //checkLevel(resources);
        toolBaseScriptService.update(resources);
    ...
    public ResponseEntity<Object> delete(@RequestBody Set<String> ids) {
        toolBaseScriptService.delete(ids);
        return FormsHttpEntity.ok();
    }
